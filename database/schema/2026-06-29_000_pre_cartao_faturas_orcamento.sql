--
-- HomeFinance - modelo anterior do schema admhomefinance
-- Versao: 2026-06-29_000
-- Gerado em: 2026-06-29
-- Origem: pg_dump --schema-only --no-owner --no-privileges --schema=admhomefinance
--
-- Este arquivo representa o modelo anterior a mudanca de
-- cartao_faturas_fechadas por orcamento.
-- Diferenca principal: cartao_faturas_fechadas nao possui orcamento_id e usa
-- chave primaria (cartao_id, mes).
-- Use para comparacao historica de modelagem, nao como script operacional.
--
...
\restrict homefinance_schema_dump
...
ALTER TABLE ONLY admhomefinance.cartao_faturas_fechadas
    ADD CONSTRAINT cartao_faturas_fechadas_pkey PRIMARY KEY (cartao_id, mes);
...
\unrestrict homefinance_schema_dump
